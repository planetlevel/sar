...
class FrameworkDetector:
    ...
    def __init__(self, project_dir: str, frameworks_dir: str = None):
        ...
        if frameworks_dir is None:
            # Get the compass package directory, then go up one level to find frameworks/
            compass_dir = os.path.dirname(os.path.abspath(__file__))
            parent_dir = os.path.dirname(compass_dir)  # Up to compass root
            frameworks_dir = os.path.join(parent_dir, 'frameworks')

            # Fallback: if frameworks/ doesn't exist at parent level, try compass package level
            if not os.path.exists(frameworks_dir):
                frameworks_dir = os.path.join(compass_dir, 'frameworks')
