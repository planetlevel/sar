...
class LibrariesAgent:
    ...
    def _get_supported_frameworks(self) -> List[str]:
        ...
        Returns list of framework IDs (filenames without .json)
        """
        # Find frameworks directory (relative to sar/ directory)
        sar_dir = os.path.dirname(os.path.dirname(__file__))
        frameworks_dir = os.path.join(sar_dir, 'frameworks')

        if not os.path.exists(frameworks_dir):
            if self.debug:
                print(f"[{self.get_agent_id().upper()}] Frameworks directory not found: {frameworks_dir}")
            return []

        # Get all .json files
        json_files = glob.glob(os.path.join(frameworks_dir, '*.json'))

        # Extract framework IDs (filename without .json)
        frameworks = [os.path.basename(f).replace('.json', '') for f in json_files]
