...
class PatternGroup(BaseModel):
    ...
    # Support old "type" field for backward compatibility in detection section
    type: Optional[Literal["filename", "filecontent", "joern", "analysis"]] = Field(
        None,
    ...
    pattern: Optional[Union[str, List[str]]] = Field(
        None,
        description="Pattern(s) to search for (annotation names, class names, etc.)"
    )

    patterns: Optional[Union[List[str], Dict[str, str]]] = Field(
        None,
        description="DEPRECATED: Use 'pattern' instead"
    )
...
class FrameworkDefinition(BaseModel):
    ...
    languages: List[str] = Field(..., min_items=1, description="Supported languages")
